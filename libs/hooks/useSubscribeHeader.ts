...
export const useSubscribeHeader = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        const unsubscribe = await api.rpc.chain.subscribeAllHeads(async (header: Header) => {
            ...
            try {
                ...
                for (let i: number=bestBlock; i>=finalizedBlock; i--) {
                    ...
                    const allRecords = (await apiAt?.query?.system?.events()).toJSON() as AnyJson[];
                ...
                console.error("An unexpected error happened when listening to new blocks from Root Network.", error);
            }
